...
| Bytecode pre-compiled at build time | Nothing may write into the bundle at runtime. Of the two ways to hold that, only this one is packaging's own job; the other would mean editing the shell's argv to serve a packaging constraint. |
